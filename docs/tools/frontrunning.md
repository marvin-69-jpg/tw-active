...
3. **baseline window 固定 20 日**——對 trending 個股可能 underestimate baseline

## v2：加 passive ETF 對照組（2026-04-25）

dump 5 檔被動 ETF（0050/0056/006208/00692/00891）持股到 `raw/cmoney/shares-passive/`，跑 `--with-passive-control`。

### 對照結果

| group   | n_evt | T mean | T median | T+1 mean | T+1 median |
|---------|------:|-------:|---------:|---------:|-----------:|
| active  | 2057  | 2.04   | **1.31** | 1.83     | 1.24       |
| passive | 133   | 2.66   | **2.12** | 1.96     | 1.45       |

**反直覺**：被動 ETF 揭露日 abnormal vol 更高。

### 解讀

被動 ETF rebalance 相對少，集中在 index reconstitution 日（每年/季少數幾次），但每次：
- magnitude 大（一次調整大量持股）
- 廣為人知（指數編製方提前公告）
- 多檔被動 ETF 同步（因為 track 同指數）

→ 自然產生強 abnormal vol。學界 Russell rebalance / MSCI inclusion effect 文獻早有記錄。

主動 ETF 揭露的 abnormal vol 反而較弱，可能來自：
1. 高頻小幅多元的調倉**稀釋每次的 front-running profit**
2. 大量「事件」混入 AP creation/redemption 雜音
3. 主動經理人本身就是 momentum follower（**反向因果**：vol 先漲 → manager 跟進買）

### 對 H1 / mechanism page 的修正

- H1 嚴格意義**不成立**——Haeberle 框架預測「強制揭露 → 額外 front-running cost」沒有比 generic ETF 機制更強
- 但「揭露日 abnormal vol > 1.0」依然存在（1.31×）
- **敘事 angle 翻轉**：主動 ETF 揭露的 cost 跟 passive 比是**較低**而非較高
- 真正可驗的精細命題改為 H4：主動 ETF 高頻 turnover × per-event front-running cost 的**累積**——可能 per-event 較小但 cumulative drag 更大

## 後續研究方向

### v3
- 拆 T-1 vs T 的 timing 分離 front-running vs reverse causality
- 加 abnormal return at T 與 reversal at T+5（連 H2 / BDR 2020）
- AUM-weighted aggregation 看 per-AUM cost 而非 per-event median

### 跨 mechanism 連結
- H2（BDR 2020 reversal）：用 `ezmoney GetPCF DIFF_UNIT × P_UNIT` 申贖量做基金層級 flow
- H4（turnover → cumulative drag）：把每次 abnormal vol 加總成 ETF 年度 implied front-running cost——v2 結果暗示這條 angle 比單事件 magnitude 更有意義
